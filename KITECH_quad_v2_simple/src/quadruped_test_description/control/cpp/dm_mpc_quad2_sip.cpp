...
void setWeightMatrices(Eigen::DiagonalMatrix<double, 4> &Q, Eigen::DiagonalMatrix<double, 1> &R)
{
    ...
    // R.diagonal() << 1e+1;
    Q.diagonal() << 100000000, 100000000000000, 10000000000, 10000000;
    R.diagonal() << 1e-6;
}
...
double cal_xl_ref(double theta_d, double acc, double theta, double *sol_xl)
{
    ...
    *sol_xl = -((A_1*acc) - B_1 + gr_xl)/((A_2*acc) + B_2);
...
int main(int argc, char **argv)
{
    ...
    // Gazebo ROS settings
    ros::init(argc, argv, "dm_test_v1_sip");
    ros::NodeHandle n;
    ...
    while (ros::ok()){        
        ...
        // slope_tau = slope_torque(pitch, 0);
        tau1.data = (ctr[0])/2; //- (yaw_vel*150);
        tau2.data = (ctr[0])/2; //+ (yaw_vel*150);
        // tau.data = ctr[0]/2;
        // F_l.data = ctr[1];
        F_l.data = sol_xl;
        // F_l.data = 0.;
        ...
        // Current States
        x0 << x_pos_current, pitch, x_dot, pitch_vel;
        xRef <<  x_pos_ref, pitch_temp, x_vel_ref, 0;
