                              ...
                              Eigen::Matrix<double, 1, 1> &uMax, Eigen::Matrix<double, 1, 1> &uMin)
{
    ...
    xMin << -OsqpEigen::INFTY, -OsqpEigen::INFTY, -4.33, -0.8;
    xMax << OsqpEigen::INFTY, OsqpEigen::INFTY, 4.33, 0.8;
}

void setWeightMatrices(Eigen::DiagonalMatrix<double, 4> &Q, Eigen::DiagonalMatrix<double, 1> &R)
{
    // force input
    // Q.diagonal() << 10000, 1000000000, 10000, 10000;
    // R.diagonal() << 1e+1;
    Q.diagonal() << 100000000, 1000000000000, 10000000000, 10000000;
    R.diagonal() << 1e+10;
}
...
double getErrorNorm(const Eigen::Matrix<double, 4, 1> &x,
...
double cal_xl_ref(double theta_d, double acc, double theta, double *sol_xl)
{
    ...
    theta_err = 0.1*(theta- theta_d);
    double xl_pos = (-(-g*h_c*m_c*sin(theta) - g*h_t*m_t*sin(theta) + h_c*m_c*cos(theta)*acc + h_t*m_t*cos(theta)*acc)/(m_t*(g*cos(theta) + sin(theta)*acc)));
...
int main(int argc, char **argv)
{
    ...
    Eigen::Matrix<double, 4, 1> x_next;
    Eigen::Matrix<double, 4, 1> x0_c;
    ...
    x0 << x_pos_current, pitch, x_dot, pitch_vel;
    xRef <<  0, 0, 0, 0;
    ...
    solver.data()->setNumberOfVariables(4 * (mpcWindow + 1) + 1 * mpcWindow);
    solver.data()->setNumberOfConstraints(2 * 4 * (mpcWindow + 1) + 1 * mpcWindow);
    ...
    while (ros::ok()){        
        ...
        ctr = QPSolution.block(4 * (mpcWindow + 1), 0, 1, 1);
        ...
        cal_xl_ref(0, 1.0, pitch, &sol_xl);

        tau1.data = (ctr[0]); //- (yaw_vel*150);
        tau2.data = (ctr[0]); //+ (yaw_vel*150);
        // tau.data = ctr[0]/2;
        // F_l.data = ctr[1];
        // F_l.data = sol_xl;
        F_l.data = 0.;
