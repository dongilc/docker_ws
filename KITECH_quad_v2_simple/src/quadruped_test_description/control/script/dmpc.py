...
#####    
def sim_trial(Tstop,Ts,Ad,Bd,Cd,F,Phi,R,Rs,Kob,Ky,Kmpc):
...
def Laguerre(N, a):
    ...
    # Calculate Al
    Al = np.block([[a, Al[-1,0:-1]]]) # 1st row
    Al = np.vstack([Al, np.block([beta, Al[-1,0:-1]])]) # 2nd row
    
    if N > 2:
        for i in range(N-2):
            multi = np.power(-a,i+1)*beta
            Al = np.vstack([Al, np.block([multi, Al[-1,0:-1]])])
    ...
    if N > 1:
        for i in range(N-1):
            L_zero[i+1] = L_zero[i]*(-a)
    
    return Al, np.sqrt(beta)*L_zero


def get_Laguerre_L(Al,L_zero,N):
    m, n = Al.shape 
    ...
    for k in range(N):   
        ...
        L_row = []
        for i in range(n):
            L_row = np.hstack([L_row, L_k.item(i)])
        #L_data.append(L_row)
        if L_data == []:
            L_data = L_row
        else:
            L_data = np.vstack([L_data, L_row])
...
def gen_Laguerre_MPC_gain(Phi, lamda, L):
    c_k_common_term = inv(L.T@Phi.T@Phi@L + lamda*L.T@L)@L.T@Phi.T
    
    return np.array(c_k_common_term)

#####    
def sim_trial_Laguerre_MPC(Tstop,Ts,Ad,Bd,Cd,F,Phi,R,Rs,Kob,Lc_k_common):
    A,B,C = gen_augmented_model(Ad, Bd, Cd)
