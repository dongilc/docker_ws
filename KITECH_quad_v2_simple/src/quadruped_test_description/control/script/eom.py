...
def tf_clean_scipy(tf, tol=1e-3):
    import copy
    ...
    for i_u in range(tf.inputs):
        for i_y in range(tf.outputs):
            num = pl.where(abs(num) < tol, pl.zeros(num.shape), num)
            den = pl.where(abs(den) < tol, pl.zeros(den.shape), den)
...
def rot_x(theta):
    rot_x = sympy.Matrix([[1,0,0],
                       [0,sympy.cos(theta),-sympy.sin(theta)],
                       [0,sympy.sin(theta),sympy.cos(theta)]])
    return rot_x

def rot_y(theta):
    rot_y = sympy.Matrix([[sympy.cos(theta),0,sympy.sin(theta)],
                       [0,1,0],
                       [-sympy.sin(theta),0,sympy.cos(theta)]])
    return rot_y

def rot_z(theta):
    rot_z = sympy.Matrix([[sympy.cos(theta),-sympy.sin(theta),0],
                       [sympy.sin(theta),sympy.cos(theta),0],
                       [0,0,1]])
    return rot_z

def trans(a,b,c):
    x = sympy.Matrix([[a],[b],[c]])
    return x
...
def get_torque_from_L(L,q,qd):
    ...
    for q_i in q:
        round_L_round_q_i = [];
        round_L_round_q_i = sympy.diff(L,q_i);
